...
export async function GET(request) {
  ...
  try {
    ...
    const showcaseSnap = await db.collection('showcasePosts')
      .where('uid', '==', uid)
      .orderBy('createdAt', 'desc')
      .limit(500)
      .get();
    const showcasePosts = showcaseSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
    ...
    const boostSnap = await db.collection('creatorBoostOrders')
      ...
      .limit(100)
      .get();
    const boostOrders = boostSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
